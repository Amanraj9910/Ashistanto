...
const { userTokenStore, conversationSessions, userPreferences, contextMemories, pool, close: closeStorage } = require('./storage');
...
const allowedOrigins = (process.env.CORS_ORIGINS || 'http://localhost:3000,http://127.0.0.1:3000,http://localhost:3001,http://127.0.0.1:3001').split(',').map((origin) => origin.trim()).filter(Boolean);
app.use(cors({ origin: (origin, callback) => !origin || allowedOrigins.includes(origin) ? callback(null, true) : callback(new Error('CORS origin not allowed')) }));
...
app.get('/health', (req, res) => res.status(200).json({ status: 'ok', service: 'ashistanto-api', timestamp: new Date().toISOString() }));
app.get('/ready', async (req, res) => {
  try {
    if (pool) await pool.query('SELECT 1');
    res.status(200).json({ status: 'ready', database: process.env.DATABASE_URL ? 'postgresql' : 'sqlite' });
  } catch (error) {
    res.status(503).json({ status: 'not_ready', error: process.env.NODE_ENV === 'production' ? 'database_unavailable' : error.message });
  }
});
  ...
  try {
    ...
    const recent = entries.slice(-12).reverse().map(([sessionId, data]) => ({
      ...
      history: (data.history || []).map((item) => ({
        ...
        text: item.content || item.user || item.assistant || ''
...
app.get('/api/preferences', async (req, res, next) => {
  try { const sessionId = String(req.query.sessionId || ''); if (!sessionId) return res.status(400).json({ error: 'sessionId is required' }); res.json(await userPreferences.getAll(sessionId)); } catch (error) { next(error); }
});
app.put('/api/preferences', async (req, res, next) => {
  try { const { sessionId, key, value } = req.body || {}; if (!sessionId || !key) return res.status(400).json({ error: 'sessionId and key are required' }); await userPreferences.set(String(sessionId), String(key), value); res.status(204).end(); } catch (error) { next(error); }
});
app.get('/api/context', async (req, res, next) => {
  try { const sessionId = String(req.query.sessionId || ''); if (!sessionId) return res.status(400).json({ error: 'sessionId is required' }); const context = await contextManager.load(sessionId); res.json({ summary: context.summary, preferences: context.preferences, memories: context.memories }); } catch (error) { next(error); }
});
app.delete('/api/context', async (req, res, next) => {
  try { const { sessionId, key } = req.body || {}; if (!sessionId || !key) return res.status(400).json({ error: 'sessionId and key are required' }); await contextManager.forget(String(sessionId), String(key)); res.status(204).end(); } catch (error) { next(error); }
});
...
app.get('/favicon.ico', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'img', 'favicon.ico.png'));
});
...
app.post('/api/process-voice', upload.single('audio'), async (req, res) => {
  ...
  try {
    ...
    const transcript = await speechToText(wavBuffer, language);
...
async function speechToText(wavBuffer, language = 'en-US') {
  return new Promise((resolve, reject) => {
    let recognizer = null;
    ...
    try {
      ...
      const supportedLanguages = new Set(['en-US', 'en-GB', 'ja-JP']);
      speechConfig.speechRecognitionLanguage = supportedLanguages.has(language) ? language : 'en-US';
...
// Extract only explicit, low-risk preferences from a user message.
async function learnUserContext(sessionId, text) {
  if (!sessionId || !text || !userPreferences) return;
  const value = String(text).trim();
  const preference = value.match(/(?:call me|address me as|refer to me as|panggil saya|sebut saya|wants? to be called|should be called|maunya disebut)\s+([a-z][a-z .'-]{1,60}?)(?:[.!?,;:]|$)/i);
  if (preference) {
    const preferredName = preference[1].replace(/[.!?,;:]+$/, '').trim().slice(0, 60);
    if (preferredName) await userPreferences.set(sessionId, 'preferred_name', preferredName);
  }
  const language = value.match(/(?:i prefer|my preferred language is|bahasa pilihan saya adalah)\s+(english\s*\(?(?:us|uk)\)?|japanese|jepang|en-us|en-gb|ja-jp)/i);
  if (language) await userPreferences.set(sessionId, 'preferred_language', language[1].toLowerCase().replace('jepang', 'japanese'));
  const timezone = value.match(/(?:my timezone is|zona waktu saya)\s+([a-z_\/+-]{3,40})/i);
  if (timezone) await userPreferences.set(sessionId, 'timezone', timezone[1].trim().slice(0, 40));
}
...
async function queryAgent(text, conversationData = { history: [], summary: '' }, sessionId = 'default', userToken = null) {
  try {
    ...
    const apiKey = process.env.AZURE_OPENAI_KEY;
    const deployment = process.env.AZURE_OPENAI_DEPLOYMENT || 'gpt-4o-mini';
    await contextManager.learn(sessionId, text);

    if (!endpoint || !apiKey) {
      if (process.env.MOCK_AI === 'true' && process.env.NODE_ENV !== 'production') {
        const lower = String(text).toLowerCase();
        if (lower.includes('email') || lower.includes('mail')) return JSON.stringify({ type: 'action_preview', preview: { to: 'Sarah Wihbow', subject: 'Schedule Changes', body: `Draft prepared from your request: ${text}` }, message: 'I prepared an email preview for your review.' });
        const mockResponse = `I understood: ${text}`;
        const mockHistory = Array.isArray(conversationData) ? conversationData : (conversationData.history || []);
        await conversationSessions.set(sessionId, [...mockHistory, { role: 'user', content: text }, { role: 'assistant', content: mockResponse }], conversationData.summary || '');
        return mockResponse;
      }
    ...
    const context = sessionId ? await contextManager.load(sessionId, conversationData) : { preferences: {}, memories: [], summary: conversationData.summary || '' };
...
Use these only when relevant. Do not invent or expose internal memory details.
If preferred_name is present, address the user with that name naturally and consistently.
...
app.post('/api/text-message', express.json(), async (req, res) => {
  try {
    ...
    const mockSessionAllowed = process.env.MOCK_AI === 'true' && process.env.NODE_ENV !== 'production';
    if (!(await userTokenStore.has(sessionId)) && !mockSessionAllowed) {
...
function shutdown(signal) {
  ...
  server.close(async () => {
    if (closeStorage) await closeStorage();
    process.exit(0);
  });
