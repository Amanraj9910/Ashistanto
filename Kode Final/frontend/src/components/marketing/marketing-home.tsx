...
import { ArrowRight, Check, ChevronRight, Eye, FileText, LockKeyhole, LogIn, Mail, Mic, Play, Send, ShieldCheck, Volume2, Zap } from 'lucide-react';
...
export function MarketingHome() {
  ...
  const [demoEmailBody, setDemoEmailBody] = useState('Hi Sarah,\n\nHere is a quick update on tomorrow\'s meeting. Please let me know if you need anything before we begin.');

  useEffect(() => {
    if (voiceDemo !== 'listening') return;
    const timer = window.setTimeout(() => setVoiceDemo('preview'), 2200);
  ...
  return (
    <main className="marketing-page">
      ...
      <section className="marketing-hero" style={{ backgroundImage: "url('/img/background-hero.png')" }}>
        ...
        <div className="hero-preview hero-voice-demo">
          <div className="preview-top"><span></span><span></span><span></span></div>
          <div className="preview-line"></div>
          <div className="voice-demo-content">
            {voiceDemo === 'idle' && <>
              <p className="voice-demo-kicker">TRY THE ASSISTANT</p>
              <h3>Say it. See it happen.</h3>
              <p className="voice-demo-prompt">Say: <strong>“Create an email to Sarah about tomorrow’s meeting.”</strong></p>
              <button type="button" className="voice-demo-mic" onClick={() => setVoiceDemo('listening')} aria-label="Start voice demo"><Mic size={26} /></button>
              <small>Click the microphone to start</small>
            </>}
            {voiceDemo === 'listening' && <>
              <p className="voice-demo-kicker">LISTENING</p>
              <h3>I'm listening…</h3>
              <div className="voice-demo-wave" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div>
              <button type="button" className="voice-demo-mic is-listening" onClick={() => setVoiceDemo('preview')} aria-label="Stop listening"><span className="voice-demo-stop"></span></button>
              <small>Click to stop · preview appears automatically</small>
            </>}
            {voiceDemo === 'preview' && <div className="voice-demo-email">
              <div className="voice-demo-email-header"><Mail size={14} /> <span>PREVIEW EMAIL</span><button type="button" onClick={() => setVoiceDemo('idle')} aria-label="Close preview">×</button></div>
              <label>TO <span>Sarah Wihbow</span></label>
              <label>SUBJECT <span>Tomorrow’s Meeting</span></label>
              <textarea value={demoEmailBody} onChange={(event) => setDemoEmailBody(event.target.value)} aria-label="Email body" />
              <div className="voice-demo-email-actions"><button type="button" onClick={() => setVoiceDemo('idle')}>Edit</button><button type="button" className="voice-demo-send" onClick={() => setVoiceDemo('sent')}>Send <Send size={14} /></button></div>
            </div>}
            {voiceDemo === 'sent' && <div className="voice-demo-sent"><div className="voice-demo-check"><Check size={24} /></div><h3>Email sent successfully</h3><p>Your message to Sarah is on its way.</p><button type="button" onClick={() => setVoiceDemo('idle')}>Try the demo again</button></div>}
          </div>
        </div>
      </section>
